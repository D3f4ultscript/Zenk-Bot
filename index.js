...
const autoClearLoop = async (channelId) => {
  try {
    ...
    if (messages.size > 0) {
      ...
      console.log(`AutoClear: Deleted ${messages.size} messages in #${channel.name}`);
    }
  } catch (e) {
    console.log(`AutoClear loop failed: ${e.message}`);
  }
};

const startAutoClear = (channelId) => {
  if (autoClearChannels.has(channelId)) return false;
  const interval = setInterval(() => autoClearLoop(channelId), 5000);
  autoClearChannels.set(channelId, interval);
  console.log(`AutoClear started for channel ${channelId}`);
  return true;
};

const stopAutoClear = (channelId) => {
  if (!autoClearChannels.has(channelId)) return false;
  clearInterval(autoClearChannels.get(channelId));
  autoClearChannels.delete(channelId);
  console.log(`AutoClear stopped for channel ${channelId}`);
  return true;
};
...
client.on('messageCreate', async (m) => {
  if (autoClearChannels.has(m.channelId) && !m.author.bot) {
    try {
      await m.delete();
      console.log(`AutoClear: Deleted instant message from ${m.author.tag} in #${m.channel.name}`);
    } catch (e) {
      console.log(`AutoClear instant delete failed: ${e.message}`);
    }
    return;
  }
...
client.on('interactionCreate', async (i) => {
  ...
  try {
    if (i.commandName === 'mute') {
      ...
      if (success) {
        await i.reply({ content: 'AutoClear activated! All new messages will be deleted instantly and 100 messages will be cleared every 5 seconds. Use `/autoclearoff` to stop.', ephemeral: true });
      } else {
        await i.reply({ content: 'AutoClear is already active in this channel', ephemeral: true });
      }
    } else if (i.commandName === 'autoclearoff') {
      if (!i.member.permissions.has(PermissionFlagsBits.ManageMessages)) return i.reply({ content: 'No permissions', ephemeral: true });
      const success = stopAutoClear(i.channelId);
      if (success) {
        await i.reply({ content: 'AutoClear deactivated', ephemeral: true });
      } else {
        await i.reply({ content: 'AutoClear is not active in this channel', ephemeral: true });
      }
    }
