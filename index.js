...
let setupConfig = {}, executionCount = 0, memberCount = 0;
...
const TIMEOUTS = { spam: 300000, linkBlock: 1200000, blacklist: 300000 };
  ...
  try {
    ...
    if (!m.webhookId) {
      ...
      if (m.channel.id !== IDS.tradeExcluded && (contentLower.includes('trade') || contentLower.includes('trading'))) {
        await m.reply({ content: 'Please use the trading channel for trades, not this channel.', allowedMentions: { repliedUser: false } }).catch(() => {});
      }
...
app.post('/execution', async (req, res) => { try { executionCount++; writeExecutions(executionCount); res.json({ success: true, count: executionCount }); } catch (e) { res.status(500).json({ success: false, error: e.message }); } });
...
app.get('/', (req, res) => res.json({ status: 'online', executions: executionCount, members: memberCount, ready: client.isReady() }));
